#!/usr/bin/env python3
"""CLI smoke script for surya/text-detection."""
...
async def main() -> int:
    parser = argparse.ArgumentParser(description="Run surya/text-detection on an image")
    ...
    runner = SuryaTextDetectionRunner()
    await runner.load(config)
    try:
        output = await runner.run(TextDetectionInput(page=page))
