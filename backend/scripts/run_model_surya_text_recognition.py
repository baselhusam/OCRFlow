#!/usr/bin/env python3
"""CLI smoke script for surya/text-recognition."""
...
async def main() -> int:
    parser = argparse.ArgumentParser(description="Run surya/text-recognition on an image")
    ...
    runner = SuryaTextRecognitionRunner()
    await runner.load(config)
    try:
        output = await runner.run(TextRecognitionInput(page=page))
