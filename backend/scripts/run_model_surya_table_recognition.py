#!/usr/bin/env python3
"""CLI smoke script for surya/table-recognition."""
...
async def main() -> int:
    parser = argparse.ArgumentParser(description="Run surya/table-recognition on an image")
    ...
    runner = SuryaTableRecognitionRunner()
    await runner.load(config)
    try:
        output = await runner.run(TableRecognitionInput(page=page))
